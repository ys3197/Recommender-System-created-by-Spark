...
$ spark-submit extension_droplow.py hdfs:/user/bm106/pub/project/cf_train.parquet hdfs:/user/bm106/pub/project/cf_validation.parquet hdfs:/path/to/save/model
'''
...
def main(spark, train_data_file, test_data_file, model_file):
    ...
    # Drop low count values less than 2:
    training_data = training_data.filter(col('count') > 1)
    # Drop low count values less than 3:
    training_data = training_data.filter(col('count') > 2)
    
    training_data = training_data.select('userindex','itemindex','count')
    testing_data = testing_data.select('userindex','itemindex','count')

    result_dict = {}
    rank_list = [500, 600, 700]
    reg_param_list = [0.7]
    alpha_list = [1]
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName('extension_droplow').config("spark.sql.broadcastTimeout","36000").getOrCreate()
